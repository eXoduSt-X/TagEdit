...
class MainActivity : AppCompatActivity(), BulkEditListener {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            for (song in selected) {
                ...
                if (result.isSuccess) {
                    ...
                    firstError = "${song.displayName}: ${result.exceptionOrNull()?.message}"
                }
            }
    ...
    override fun onConvertTagToFilename(pattern: String) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            for (song in selected) {
                ...
                if (tags == null) {
                    if (firstError == null) {
                        firstError = "Leyendo ${song.displayName}: ${tagsResult.exceptionOrNull()?.message}"
                    }
                    continue
                }
                ...
                if (renameResult.isSuccess) {
                    ...
                    firstError = "Renombrando ${song.displayName}: ${renameResult.exceptionOrNull()?.message}"
                }
            }
            withContext(Dispatchers.Main) {
    ...
    /** Muestra un Toast con el conteo de éxito y, si hubo algún fallo, el primer error concreto. */
    private fun showResult(action: String, success: Int, total: Int, firstError: String?) {
        val base = "$action: $success de $total archivo(s)"
        val message = if (firstError != null) "$base\nError: $firstError" else base
