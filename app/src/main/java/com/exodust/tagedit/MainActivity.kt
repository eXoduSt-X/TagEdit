...
class MainActivity : AppCompatActivity(), BulkEditListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.buttonBulkEdit).setOnClickListener {
            ...
            dialog.selectedSongs = adapter.getSelectedSongs()
    ...
    override fun onApplyArtwork(imageBytes: ByteArray, mimeType: String) {
        val selected = adapter.getSelectedSongs()
        lifecycleScope.launch(Dispatchers.IO) {
            var success = 0
            var firstError: String? = null
            for (song in selected) {
                val result = TagIO.writeArtwork(applicationContext, song, imageBytes, mimeType)
                if (result.isSuccess) {
                    success++
            ...
            withContext(Dispatchers.Main) {
                showResult("Portada aplicada", success, selected.size, firstError)
            }
        }
    }
    ...
    override fun onPreviewTagToFilename(pattern: String) {
        val selected = adapter.getSelectedSongs()
        lifecycleScope.launch(Dispatchers.IO) {
            val lines = mutableListOf<String>()
            for (song in selected) {
                val tags = TagIO.readTags(applicationContext, song).getOrNull()
                if (tags == null) {
                    lines += "${song.displayName}\n  → (no se pudo leer el tag)"
                    continue
                }
            ...
            withContext(Dispatchers.Main) {
                ...
                AlertDialog.Builder(this@MainActivity)
                    .setTitle("Vista previa (todavía no se renombró nada)")
                    .setView(android.widget.ScrollView(this@MainActivity).apply { addView(messageView) })
